...
### 🔀 Week-6: Version Control with Git & GitLab
- **Git Configuration & Setup**: Initializing repositories, configuring user settings, and setting up the local environment.
- **Tracking & Committing**: Working with `git add`, `git commit`, `git status`, and ignoring files via `.gitignore`.
- **Branching & Merging**: Creating branches, switching branches, merging branches, and resolving 3-way merge conflicts using P4Merge.
- **Remote Repositories**: Pushing code, pulling changes, resolving sync issues, and maintaining up-to-date remotes on GitLab/GitHub.
...
### Running the Backend Services 🖥️
